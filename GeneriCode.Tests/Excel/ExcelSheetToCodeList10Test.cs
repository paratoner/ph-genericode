...
{
    [TestClass]
    public class ExcelSheetToCodeList10Test : TestBase
    {
        private static TestContext _testContext;

        public ExcelSheetToCodeList10Test() : base(_testContext)
        {
        ...
        public void TestReadExcel()
        {
            foreach (string aFile in Directory.GetFiles(Path.Combine(ResourceFolderPath, @"excel")))
            {
                ...
                using (FileStream file = new FileStream(aFile, FileMode.Open, FileAccess.Read))
                {
                    ...
                    CodeListDocument aCodeList = ExcelSheetToCodeList10.ConvertToSimpleCodeList(aSheet,
                                                                                                     aReadOptions,
                                                                                                      "ExampleList",
                                                                                                      "1.0",
                                                                                                      ...
                                                                                                      null);
                    string aDoc = new Genericode10CodeListSerializer().Serialize(aCodeList);
                    Assert.IsNotNull(aDoc);
                    CodeListDocument aCLDoc = new Genericode10CodeListSerializer().Deserialize(aDoc);
